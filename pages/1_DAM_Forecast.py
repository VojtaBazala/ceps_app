...
st.markdown(
    ...
    f'Forecast pro: <span style="color:#00e676">{forecast_date}</span></div>',
    unsafe_allow_html=True
)
...
with col2:
    ...
    if not df_summary.empty:
        ...
        if all(v is not None for v in [mae, rmse, median, smape]):
            ...
            html3 = '<div style="color:#8899bb;font-size:.8rem;">Zatím nedostatek dat</div>'
...
if not df_forecast.empty:
    ...
    # Skutečné ceny pokud jsou dostupné
...
# ── GRAF: FORECAST TRACKING – posledních 5 dní ────
st.markdown(
...
if not df_hist.empty and not df_prices.empty:
    ...
    # Dny kde máme obojí – forecast i skutečnost
        ...
        for i, day in enumerate(common_dates, start=1):
            ...
            if not fc_day.empty:
                fig2.add_trace(go.Scatter(
                    x=fc_day["hour"],
                    y=fc_day["forecast_price"],
                    name=f"Forecast {day.strftime('%d.%m')}",
            ...
            if not ac_day.empty:
                fig2.add_trace(go.Scatter(
                    x=ac_day["hour"],
                    y=ac_day["price_eur"],
                    name=f"Skutečnost {day.strftime('%d.%m')}",
            ...
            if not fc_day.empty and not ac_day.empty:
                ...
                if not merged.empty:
                    ...
                    fig2.add_annotation(
                        ...
                        x=23, y=merged[["forecast_price","price_eur"]].max().max(),
                        showarrow=False,
