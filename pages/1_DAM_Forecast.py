...
if st.session_state.dark_mode:
    BG=="#0a0e1a"; BORDER="#1e2d50"; TEXT="#cdd8f0"; SUBTEXT="#8899bb"
    PLOT_BG="#0f1628"; PAPER_BG="#0a0e1a"; GRID_COL="#1e2d50"; FONT_COL="#cdd8f0"; LEG_COL="#cdd8f0"; BTN_TEMA="☀️"
    BG="#0a0e1a"
else:
    BG="#f5f7fa"; BORDER="#dde3ef"; TEXT="#1a2035"; SUBTEXT="#6677aa"
    PLOT_BG="#ffffff"; PAPER_BG="#f5f7fa"; GRID_COL="#dde3ef"; FONT_COL="#1a2035"; LEG_COL="#1a2035"; BTN_TEMA="🌙"

st.markdown(f"""
<style>
  #MainMenu {{visibility:hidden;}} footer {{visibility:hidden;}} header {{visibility:hidden;}}
  .block-container {{ padding-top:1.2rem; background:{BG}; }} .stApp {{ background:{BG}; }}
  .page-title {{ font-family:'Courier New',monospace; font-size:1.8rem; font-weight:700; color:#00e676; letter-spacing:3px; text-transform:uppercase; margin-bottom:0.3rem; }}
  .divider {{ border-top:1px solid {BORDER}; margin:0.8rem 0; }}
  .col-header {{ font-family:'Courier New',monospace; font-size:0.7rem; letter-spacing:3px; text-transform:uppercase; border-bottom:2px solid; padding-bottom:6px; margin-bottom:12px; }}
  .col-header.green {{ border-color:#00e676; color:#00e676; }}
  .col-header.blue  {{ border-color:#00c8ff; color:#00c8ff; }}
  .row-item {{ display:flex; justify-content:space-between; align-items:center; padding:5px 0; border-bottom:1px solid {BORDER}; font-family:'Courier New',monospace; }}
...
with col2:
    ...
    if not df_summary.empty:
        ...
        mae=latest.get("mae_all"); rmse=latest.get("rmse_all"); median=latest.get("median_abs_error"); smape=latest.get("smape_all_pct"); days=latest.get("eval_days_total",0)
...
fig = go.Figure()
fig.add_trace(go.Bar(x=df_forecast["hour"], y=df_forecast["forecast_price"], name="Forecast", marker_color="#00e676", hovertemplate="Hodina %{x}:00<br><b>%{y:.2f} EUR/MWh</b>"))
if not df_prices.empty:
    ...
    if not actual.empty:
        fig.add_trace(go.Scatter(x=actual["hour"], y=actual["price_eur"], name="Skutečná cena", line=dict(color="#ff3d57", width=2), hovertemplate="Hodina %{x}:00<br><b>%{y:.2f} EUR/MWh</b>"))
fig.update_layout(**base_layout("Forecast cen – zítřek [EUR/MWh]", "#00e676"))
fig.update_xaxes(tickmode="linear", tick0=0, dtick=1, ticktext=[f"{h:02d}:00" for h in range(24)], tickvals=list(range(24)))
st.plotly_chart(fig, use_container_width=True)

st.markdown('<div class="divider"></div>', unsafe_allow_html=True)
st.markdown(
    f'<div style="font-family:\'Courier New\',monospace;font-size:0.7rem;letter-spacing:3px;text-transform:uppercase;color:#00c8ff;border-bottom:2px solid #00c8ff;padding-bottom:6px;margin-bottom:16px;">📅 Forecast vs. Skutečnost – posledních 5 dní</div>',
    unsafe_allow_html=True
)
...
if not df_hist.empty and not df_prices.empty:
    ...
    common_dates = sorted([d for d in df_hist["forecast_date"].unique() if d in set(df_prices["price_date"].unique())], reverse=True)[:5]
    if common_dates:
        n = len(common_dates)
        fig2 = make_subplots(rows=n, cols=1, subplot_titles=[d.strftime("%d.%m.%Y") for d in common_dates], vertical_spacing=0.08)
        for i, day in enumerate(common_dates, start=1):
            ...
            if not fc_day.empty:
                fig2.add_trace(go.Scatter(x=fc_day["hour"], y=fc_day["forecast_price"], name="Forecast", line=dict(color="#00e676",width=1.5), hovertemplate=f"{day.strftime('%d.%m')} %{{x}}:00<br>Forecast: <b>%{{y:.2f}} EUR</b>", showlegend=(i==1)), row=i, col=1)
            if not ac_day.empty:
                fig2.add_trace(go.Scatter(x=ac_day["hour"], y=ac_day["price_eur"], name="Skutečnost", line=dict(color="#ff3d57",width=1.5,dash="dot"), hovertemplate=f"{day.strftime('%d.%m')} %{{x}}:00<br>Skutečnost: <b>%{{y:.2f}} EUR</b>", showlegend=(i==1)), row=i, col=1)
            if not fc_day.empty and not ac_day.empty:
                ...
                if not merged.empty:
                    ...
                    fig2.add_annotation(text=f"MAE: {mae_day:.1f} EUR/MWh", xref=f"x{i}", yref=f"y{i}", x=23, y=merged[["forecast_price","price_eur"]].max().max(), showarrow=False, font=dict(size=9,color="#ffd740",family="Courier New"), xanchor="right")
        fig2.update_layout(paper_bgcolor=PAPER_BG, plot_bgcolor=PLOT_BG, font=dict(color=LEG_COL,family="Courier New",size=11), hovermode="x unified", height=180*n, margin=dict(l=50,r=10,t=30,b=20), legend=dict(bgcolor=PLOT_BG,bordercolor=BORDER,font=dict(size=11,color=LEG_COL),orientation="h",y=1.02,x=0))
...
with st.expander("📋 Hodnocení přesnosti – posledních 30 dní"):
    if not df_eval.empty: st.dataframe(df_eval, use_container_width=True, hide_index=True)
    else: st.info("Zatím žádná hodnocení.")
