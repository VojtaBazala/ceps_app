...
<style>
  ...
  .page-title {{
    ...
    font-weight: 700; color: #ffd740; letter-spacing: 3px;
...
with header_r:
    ...
    with nav_sel:
        nav = st.selectbox(
            "", ["— Přejít na —", "⚡ ČEPS online", "📈 DAM Forecast"],
            key="nav_mfrr", label_visibility="collapsed"
        )
        if nav == "⚡ ČEPS online":
            st.switch_page("CEPS_online.py")
        elif nav == "📈 DAM Forecast":
...
st.markdown(
    ...
    f' &nbsp;|&nbsp; Počet nabídek: <span style="color:#ffd740">{len(df)}</span>'
    f'</div>',
    unsafe_allow_html=True
)
...
st.dataframe(
    ...
    hide_index=True,
    column_config={
        "Timeseries ID":     st.column_config.TextColumn(width="small"),
