...
<style>
  ...
  .tbl-info {{ font-family:'Courier New',monospace; font-size:0.75rem; color:{SUBTEXT}; margin-bottom:8px; }}
...
st.markdown(
    ...
    f'Hodnoty pro datum dodávky &nbsp;|&nbsp; mFRR+: <span style="color:#00e676">{date_plus}</span>'
    f' &nbsp;/&nbsp; mFRR-: <span style="color:#ff3d57">{date_minus}</span>'
    f'</div>',
    unsafe_allow_html=True
)
...
with col_plus:
    ...
    if not df_plus.empty:
        ...
        df_show = df_plus[["product_type","position","quantity_mw","price_eur_mw","cum_quantity_mw"]].copy()
        df_show.columns = ["Product Type","Position","Quantity [MW]","Price [EUR/MW]","Cum. Qty [MW]"]
        st.dataframe(df_show.round(2), use_container_width=True, hide_index=True, column_config=COL_CONFIG)
    else:
...
with col_minus:
    ...
    if not df_minus.empty:
        ...
        df_show = df_minus[["product_type","position","quantity_mw","price_eur_mw","cum_quantity_mw"]].copy()
        df_show.columns = ["Product Type","Position","Quantity [MW]","Price [EUR/MW]","Cum. Qty [MW]"]
        st.dataframe(df_show.round(2), use_container_width=True, hide_index=True, column_config=COL_CONFIG)
    else:
        st.warning("Žádná mFRR- data")
...
if not df_plus.empty or not df_minus.empty:
    ...
    fig.update_layout(
        ...
        height=500, margin=dict(l=80, r=20, t=50, b=50),
        ...
        yaxis=dict(title="Cena [EUR/MW]", gridcolor=GRID_COL, color=LEG_COL),
        annotations=[
