...
try:
    with engine.connect() as conn:
        result = conn.execute(text("SELECT COUNT(*) FROM mfrr_minus_orderbook WHERE trade_date = :d"), {"d": delivery_date})
        count = result.scalar()
    if count > 0:
...
with engine.connect() as conn:
    conn.execute(text("""
        CREATE TABLE IF NOT EXISTS mfrr_minus_orderbook (
    ...
    conn.execute(text("DELETE FROM mfrr_minus_orderbook WHERE trade_date = :d"), {"d": delivery_date})
    for _, r in df_result.iterrows():
        conn.execute(text("""
            INSERT INTO mfrr_minus_orderbook
